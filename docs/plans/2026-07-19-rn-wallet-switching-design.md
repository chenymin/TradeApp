...
采用“外部连接器 + Privy SIWE 归属 + 独立 `wallet-select` + 数据库原子 RPC + 会话替换”的受控流程：
...
6. Edge Function 根据数据库最终地址签发同一 investor 的替换 JWT 与 `AuthViewer`。移动端先持久化 session，再显示完整成功。
...
| ---------------- | -------- | ------------ |
| 已确认：绑定新钱包后立即 active，与当前 Web 端用户可见行为保持一致；旧的“绑定后只加入列表、不自动 active”方案已废弃，不得出现在后续设计或计划中。 | Resolved：SIWE link 成功后工作流自动进入 `wallet-select` 与 session 持久化；只有端到端完成才显示成功。 | No |
...
| 已确认：本功能不修改现有 `wallet-login`，新增最小 `wallet-select` 路径承载 active 选择、数据库原子更新与新会话签发。 | Resolved：新增独立 Edge Function、workflow 模块和 RPC；不修改 `wallet-login/index.ts`。 | No |
...
| 非阻塞：新 JWT 是同一账户的替换短期会话，用于让 `AuthViewer.walletAddress` 与新平台 active wallet 一致；不是创建新账户。 | Resolved：JWT `sub` 继续使用原 investor id，只改变返回 Viewer 的 wallet address；不调用创建账户分支。 | No |
...
| `wallet-select` 独立于 `wallet-login` | 现有登录函数按数组首项选 primary，且用户要求暂不改其逻辑 | 新函数必须自行验证 Privy、调用原子 RPC并签发替换 session；不复用错误的 primary 选择 |
...
- 移动端服务边界：`walletSelectClient.ts` 只负责请求 / 响应解析；`createWalletSelectionDependencies.ts` 只适配 hooks、Alert、session 替换和 public config。
- Edge Function 入口：`wallet-select/index.ts` 只处理 CORS、HTTP input、调用 service 并映射稳定错误码。
- Edge Function 领域 / service：请求解析、linked-target 选择、investor 状态检查、RPC 调用、JWT / Viewer 响应分别为小函数；不在入口堆叠大型 `if/else`。
...
- `walletSelectionWorkflow`：按状态调用注入依赖；operation id 由注入 factory 生成；可用 fake connector / Privy / platform / session adapter 测试每个失败点。
...
- `walletSelectClient`：解析稳定 response / error union，不向 UI泄漏 token 或服务端原始 body。
- `wallet-select` server domain：规范化地址、查找明确 linked target、错误分类为纯函数；HTTP、Privy fetch、Supabase RPC 和 JWT 签发通过依赖隔离。
- 数据库 RPC：用 migration SQL 和本地 Supabase / transaction integration 验证唯一性、锁顺序、回滚和幂等。
- 副作用边界：连接 / 签名仅在 adapter；Privy link 仅在 Privy adapter；平台请求仅在 client；session 修改仅在 AuthProvider action；数据库写仅在 RPC。
...
8. Edge Function按 RPC 最终地址签发同一 investor 的短期 JWT 与 Viewer。
9. 移动端将 session 写入现有 secure storage 与 Supabase client，AuthProvider 更新 Viewer；三者地址一致后显示 `Active`。
...
4. 后续平台选择与 session 替换同绑定路径一致，不调用 Privy SIWE link。

### 平台响应未知或 session 持久化失败

1. 保留 operation id、target、expected previous 和阶段，不保留 token / JWT。
2. 对相同 operation id 重调 `wallet-select`。RPC 若已有 completed operation，返回其最终地址；若未提交，则执行一次事务。
3. 新 JWT 可重新签发；客户端只重复 session 持久化，不重复连接、签名或 SIWE link。
...
- 状态模型：`idle -> connecting/linking/confirming -> platform_syncing -> session_persisting -> complete`；错误分为可重试 `connect_error` / `bind_error` / `sync_error` / `session_sync_pending` 与锁定态 `consistency_error`。
...
| 配置密钥 / 兼容回滚 | 适用 | Reown project id 和 endpoint path 为 public config；service role / JWT secret 仅 Edge Function；客户端功能可 feature-disable，数据库 migration 向后兼容旧读路径 |
...
| session 持久化失败 | 平台可能已切换，本机 Viewer 仍旧 | `session_sync_pending`；重调同 operation 获取新 JWT，只重试 session 写入 |
...
- Secret / token：Privy token 和新 JWT 只在请求 / secure session 流中存在；日志只允许 operation id、稳定错误码和脱敏地址；禁止输出 request body、Privy response 或 Authorization header。
