...
Generated：2026-07-15T06:20:50.052Z
Status：PASS
Pass / Total：8 / 8
...
| 验证目标 | 命令 | 状态 | 耗时 |
| -------- | ---- | ---- | ---- |
| TypeScript | `npx tsc --noEmit` | pass | 2210ms |
| 全量单元测试 | `npm test -- src` | pass | 1516ms |
| ai-delivery 审核 | `npm run ai:audit -- rn-full-app-port` | pass | 105ms |
...
| 禁止 UI 直接写表 | `node scripts/verify-no-match.mjs -e "\\.from\\(.+\\)\\.insert" -e "\\.from\\(.+\\)\\.upsert" -e "\\.from\\(.+\\)\\.update" -e "\\.from\\(.+\\)\\.delete" src/ --glob "*.tsx"` | pass | 27ms |
