...
- 颜色必须有语义：祖母绿用于主操作、选中态和正向状态；危险色只用于失败和破坏性操作；香槟色仅作为低频品牌强调，不承担主要操作。Dashboard 业务 Tab 的当前选项必须使用祖母绿实色底和白色文字，不得只依赖浅色填充、模糊或阴影表达选中。
...
| 场景 | Given | When | Then |
| ---- | ----- | ---- | ---- |
| Dashboard 正常数据 | 已登录投资者拥有 portfolio、PnL、等级、积分、KYC 与持仓数据 | 打开 Dashboard | 首屏只有一个核心资产焦点，辅助指标层级较低，用户无需滚动即可识别总资产、PnL、当前 Tab 与首个内容区；当前 Tab 以祖母绿实色底和白字与其他选项明确区分 |
